...
--[[
	...
	different table. That breaks identity comparisons — fire a player's tool table
	through a BindableEvent and the listener cannot tell it is the same table —
	and it costs real performance in a hot path. This is a plain Lua
	implementation with no copy.
...
--[[
	...
	or yielding cannot stop the others. A single bad listener silently swallowing
	every subsequent kill event is the kind of bug that takes a day to find.
]]
