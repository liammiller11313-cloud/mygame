...
AudioConfig.WeaponFire = {
	...
	[Enums.Weapon.Machete] = sound(EMPTY, 0.55, 0.9, 1.1, 60, 3),
...
AudioConfig.Impact = {
	...
	Concrete = sound(EMPTY, 0.5, 0.9, 1.12, 110, 1),
	Metal = sound(EMPTY, 0.6, 0.9, 1.15, 130, 1),
	Wood = sound(EMPTY, 0.5, 0.9, 1.12, 110, 1),
	Glass = sound(EMPTY, 0.65, 0.92, 1.1, 140, 2),
	Water = sound(EMPTY, 0.45, 0.92, 1.08, 100, 1),
	Dirt = sound(EMPTY, 0.45, 0.9, 1.12, 100, 1),
} :: { [string]: SoundDefinition }
...
AudioConfig.Gore = {
	...
	BodyFall = sound(EMPTY, 0.5, 0.9, 1.1, 90, 2),
...
AudioConfig.Survivor = {
	Hurt = sound(EMPTY, 0.6, 0.95, 1.05, 80, 4),
	Incap = sound(EMPTY, 0.8, 1.0, 1.0, 160, 6),
	Death = sound(EMPTY, 0.8, 1.0, 1.0, 180, 6),
	Revived = sound(EMPTY, 0.6, 1.0, 1.0, 90, 4),
	HealSelf = sound(EMPTY, 0.55, 1.0, 1.0, 60, 3),
	PillsUse = sound(EMPTY, 0.5, 1.0, 1.0, 50, 3),
	Breathing = sound(EMPTY, 0.5, 1.0, 1.0, 40, 2),
...
AudioConfig.UI = {
	Pickup = sound(EMPTY, 0.5, 1.0, 1.0, 30, 3),
	PromptAppear = sound(EMPTY, 0.3, 1.0, 1.0, 30, 1),
	ObjectiveChange = sound(EMPTY, 0.6, 1.0, 1.0, 30, 4),
	WaveIncoming = sound(ID.ZombieHorde, 0.55, 0.9, 0.95, 30, 6),
	WaveCleared = sound(EMPTY, 0.7, 1.0, 1.0, 30, 6),
	-- Deliberately reusing the impact samples, pitched way up and played quietly:
	-- the hitmarker tick and the flesh thud reading as the same event is exactly
	-- the association we want, and it costs nothing.
	Hitmarker = sound(ID.BodyShot1, 0.22, 1.7, 1.8, 20, 2),
	HeadshotMarker = sound(ID.Headshot, 0.3, 1.5, 1.6, 20, 3),
} :: { [string]: SoundDefinition }
...
AudioConfig.Music = {
	...
	WaveCleared = { id = EMPTY, volume = 0.5, fadeIn = 0.2, fadeOut = 1.5, looped = false, loopCrossfade = 0 },
	Defeat = { id = EMPTY, volume = 0.5, fadeIn = 0.5, fadeOut = 2.0, looped = false, loopCrossfade = 0 },
	Victory = { id = EMPTY, volume = 0.5, fadeIn = 0.5, fadeOut = 2.0, looped = false, loopCrossfade = 0 },
