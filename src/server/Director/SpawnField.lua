...
function SpawnField.stats(): { known: number, explored: number, total: number, done: boolean, condemned: number }
	local bad = 0
	for _ in condemned do
		bad += 1
	end
	return { known = #list, explored = cursor, total = cellCount, done = swept, condemned = bad }
end
